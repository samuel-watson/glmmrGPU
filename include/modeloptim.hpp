...
inline glmmr::ModelOptim<modeltype>::ModelOptim(modeltype& model_,
    ...
    gradients(model.linear_predictor.P() + model.covariance.npar()) {
}; //ll_previous(ArrayXXd::Zero(re_.mcmc_block_size,2)), 
...
inline bool glmmr::ModelOptim<modeltype>::check_convergence(const double tol, const int hist, const int k, const int k0) {
    ...
    std::cout << "BF: " << bf << " prior: " << prior0 << std::endl;
    double meang = 0;
    for (const double x : gradient_history) meang += x;
    meang *= 1.0 / gradient_history.size();
    double diff = quantile == 0 ? 10 : meang - quantile;

    std::cout << "Gradients: " << gradients.transpose() << " | ||G|| = " << gradients.matrix().norm() << std::endl;
    std::cout <<  "Log-likelihood running mean: " << meang << " (old " << quantile << ") diff: " << diff << std::endl;
    quantile = meang;
    return bf > tol;
}
